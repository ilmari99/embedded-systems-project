...
void KEYPAD_WaitForKeyPress(int* has_timed_out)
{
	...
	setup_timer1();
	int start_count = overflow_count;
	do
	{
		do
		{
			...
			int time_diff = overflow_count - start_count;
			if (time_diff > 5){
				*has_timed_out = 1;
                return;
			}
		}
		while(var_keyPress_u8==0x0F); // Wait till the Key is pressed, at most 10 seconds
...
uint8_t KEYPAD_GetKey(int* has_timed_out)
{
	...
	KEYPAD_WaitForKeyPress(has_timed_out);      // Wait for the new key press
	if (*has_timed_out == 1){
		return 'A';
	}
...
static uint8_t keypad_ScanKey()
{
	...
	// Return "A" if no key press is detected
	if(var_keyPress_u8==0x0F)
		var_keyPress_u8=0x7e;
